...
class Unit2ConstantsAndInitTest {
    ...
    void driveMotorCurrentLimit() throws Exception {
        ...
        assertTrue(Modifier.isPublic(f.getModifiers()), "DRIVE_MOTOR_CURRENT_LIMIT must be public");
        assertTrue(Modifier.isStatic(f.getModifiers()), "DRIVE_MOTOR_CURRENT_LIMIT must be static");
    ...
    @BeforeAll
    static void initHal() {
        // HAL.initialize returns false (not throws) if already initialized — safe to call in multiple test classes.
    ...
    void constructorDoesNotThrow() {
        assertDoesNotThrow(
                ...
                "CANDriveSubsystem constructor threw an exception — check that all TalonFX objects are created and TalonFXConfiguration is applied");
    }

    @Test
    @DisplayName("Unit 2: drive field is non-null after construction")
    void driveFieldIsNonNull() throws Exception {
        CANDriveSubsystem subsystem = new CANDriveSubsystem();
        Field driveField = CANDriveSubsystem.class.getDeclaredField("drive");
        driveField.setAccessible(true);
        assertNotNull(
                driveField.get(subsystem),
                "drive field must not be null — initialize it with new DifferentialDrive(leftLeader, rightLeader)");
    }
}
